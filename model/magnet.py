...
class MagNet3Frames(object):
    def __init__(self, sess, name, arch_config):
        ...
        self.num_texture_resblk = arch_config["ynet_3frames"]["num_texture_resblk"]
        self.texture_dims = arch_config["ynet_3frames"]["texture_dims"]
        self.texture_downsample = arch_config["ynet_3frames"]["texture_downsample"]
        self.use_texture_conv = arch_config["ynet_3frames"]["use_texture_conv"]
        self.shape_dims = arch_config["ynet_3frames"]["shape_dims"]
        self.num_shape_resblk = arch_config["ynet_3frames"]["num_shape_resblk"]
        self.use_shape_conv = arch_config["ynet_3frames"]["use_shape_conv"]
        self.decoder_dims = self.texture_dims + self.shape_dims
        self.probe_pt = {}
        self.manipulator = partial(res_manipulator,layer_dims=self.encoder_dims, num_resblk=self.num_man_resblk, num_conv=self.num_man_conv,
                                num_aft_conv=self.num_man_aft_conv, probe_pt=self.probe_pt)

    def _encoder(self, image):
        enc = res_encoder(image, layer_dims=self.encoder_dims, num_resblk=self.num_enc_resblk)
        texture_enc ,shape_enc  = enc ,enc
        # first convolution on common encoding
        if self.use_texture_conv:
            stride = 2 if self.texture_downsample else 1
            texture_enc = tf.nn.relu(conv2d(texture_enc, self.texture_dims, 3, stride, name='enc_texture_conv'))
        else:
            assert self.texture_dims == self.encoder_dims, \
                "Texture dim ({}) must match encoder dim ({}) " \
                "if texture_conv is not used.".format(self.texture_dims, self.encoder_dims)
            assert not self.texture_downsample, "Must use texture_conv if texture_downsample."

        if self.use_shape_conv:
            shape_enc = tf.nn.relu(conv2d(shape_enc, self.shape_dims, 3, 1, name='enc_shape_conv'))
        else:
            assert self.shape_dims == self.encoder_dims, \
                "Shape dim ({}) must match encoder dim ({}) " \
                "if shape_conv is not used.".format(self.shape_dims, self.encoder_dims)
        ...
        for i in range(self.num_texture_resblk):
            ...
            if i == 0:
                ...
                name = 'texture_enc'
            texture_enc = residual_block(texture_enc, self.texture_dims, 3, 1,name)
    ...
    def _decoder(self, texture_enc, shape_enc):
        if self.texture_downsample:
            texture_enc = tf.compat.v1.image.resize_nearest_neighbor( texture_enc, tf.shape(texture_enc)[1:3] * 2)
            texture_enc = tf.pad(texture_enc, [[0, 0], [1, 1], [1, 1], [0, 0]], "REFLECT")
            texture_enc = tf.nn.relu(conv2d(texture_enc, self.texture_dims, 3, 1, padding='VALID', name='texture_upsample'))

        enc = tf.concat([texture_enc, shape_enc], axis=3)
        # Needs double the channel because we concat the two encodings.
        return res_decoder(enc, layer_dims=self.decoder_dims, out_channels=self.n_channels, num_resblk=self.num_dec_resblk)

    def image_transformer(self, image_a, image_b, amplification_factor, im_size, options, is_training, reuse=False, name='ynet_3frames'):
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            ...
            with tf.compat.v1.variable_scope('manipulator'):
                self.out_shape_enc = self.manipulator(self.shape_a, self.shape_b, amplification_factor)
    ...
    def _build_feed_model(self):
        self.test_input = tf.compat.v1.placeholder(tf.float32, [None, None, None, self.n_channels * 3],  name='test_AB_and_output')
        self.test_amplification_factor = tf.compat.v1.placeholder(tf.float32, [None], name='amplification_factor')
        ...
        self.test_output = self.image_transformer(self.test_image_a, self.test_image_b, self.test_amplification_factor, [self.image_height, self.image_width],
                               self.arch_config,False, False)
    ...
    def inference(self, frameA, frameB, amplification_factor):
        ...
        """
        in_frames = [load_train_data([frameA, frameB, frameB], gray_scale=self.n_channels==1, is_testing=True)]
        in_frames = np.array(in_frames).astype(np.float32)
        out_amp = self.sess.run(self.test_output, feed_dict={self.test_input: in_frames, self.test_amplification_factor: [amplification_factor]})
        return out_amp

    def run(self, checkpoint_dir, vid_dir, frame_ext, out_dir,amplification_factor, velocity_mag=False):
    ...
    def _build_IIR_filtering_graphs(self):
        ...
        Assume a_0 = 1
        """
        self.input_image = tf.compat.v1.placeholder(tf.float32,[1, self.image_height, self.image_width, self.n_channels], name='input_image')
        self.filtered_enc = tf.compat.v1.placeholder(tf.float32, [1, None, None, self.shape_dims], name='filtered_enc')
        self.out_texture_enc = tf.compat.v1.placeholder(tf.float32,[1, None, None, self.texture_dims], name='out_texture_enc')
        self.ref_shape_enc = tf.compat.v1.placeholder(tf.float32, [1, None, None, self.shape_dims], name='ref_shape_enc')
        self.amplification_factor = tf.compat.v1.placeholder(tf.float32, [None], name='amplification_factor')
        with tf.compat.v1.variable_scope('ynet_3frames'):
            with tf.compat.v1.variable_scope('encoder'):
                self.texture_enc, self.shape_rep = self._encoder(self.input_image)
            with tf.compat.v1.variable_scope('manipulator'):
                # set encoder a to zero because we do temporal filtering
                # instead of taking the difference.
                self.out_shape_enc = self.manipulator(0.0, self.filtered_enc,self.amplification_factor)
                self.out_shape_enc += self.ref_shape_enc - self.filtered_enc
            with tf.compat.v1.variable_scope('decoder'):
                self.output_image = tf.clip_by_value( self._decoder(self.out_texture_enc, self.out_shape_enc), -1.0, 1.0)
        self.saver = tf.compat.v1.train.Saver()

    def run_temporal(self, checkpoint_dir, vid_dir, frame_ext, out_dir, amplification_factor, fl, fh, fs, n_filter_tap, filter_type):
        ...
        if filter_type == 'fir':
            ...
            filter_b, filter_a = butter(n_filter_tap, [fl/nyq, fh/nyq], btype='bandpass')
            filter_a = filter_a[1:]
            ...
            raise ValueError('Filter type must be either '  '["fir", "butter", "differenceOfIIR"] got ' + filter_type)
        head, tail = os.path.split(out_dir)
        tail = tail + '_fl{}_fh{}_fs{}_n{}_{}'.format(fl, fh, fs, n_filter_tap, filter_type)
        ...
        if len(filter_a) !=0:
            ...
            for frame in tqdm(vid_frames, desc='Applying IIR'):
                ...
                in_frames = [load_train_data([frame, frame, frame], gray_scale=self.n_channels==1, is_testing=True)]
                in_frames = np.array(in_frames).astype(np.float32)
                texture_enc, x = self.sess.run([self.texture_enc, self.shape_rep], feed_dict={ self.input_image:in_frames[:, :, :, :3],})
                x_state.insert(0, x)
                ...
                out_amp = self.sess.run(self.output_image, feed_dict={self.out_texture_enc: texture_enc, self.filtered_enc: y, self.ref_shape_enc: x,
            ...
            # convolution.
            x_state = None
            for i, frame in tqdm(enumerate(vid_frames), desc='Getting encoding'):
                file_name = os.path.basename(frame)
                in_frames = [load_train_data([frame, frame, frame], gray_scale=self.n_channels==1, is_testing=True)]
                in_frames = np.array(in_frames).astype(np.float32)
                texture_enc, x = self.sess.run([self.texture_enc, self.shape_rep], feed_dict={self.input_image: in_frames[:, :, :, :3],})
                if x_state is None:
                    x_state = np.zeros(x.shape + (len(vid_frames),), dtype='float32')
                x_state[:, :, :, :, i] = x

            filter_fft = np.fft.fft(np.fft.ifftshift(filter_b), n=x_state.shape[-1])
            # Filtering
            ...
            for i, frame in tqdm(enumerate(vid_frames), desc='Decoding'):
                ...
                in_frames = [load_train_data([frame, frame, frame],gray_scale=self.n_channels==1, is_testing=True)]
                in_frames = np.array(in_frames).astype(np.float32)
                texture_enc, _ = self.sess.run([self.texture_enc, self.shape_rep],feed_dict={ self.input_image: in_frames[:, :, :, :3],})
                out_amp = self.sess.run(self.output_image, feed_dict={self.out_texture_enc: texture_enc, self.filtered_enc: x_state[:, :, :, :, i], self.ref_shape_enc: x, self.amplification_factor: [amplification_factor]})
                im_path = os.path.join(out_dir, file_name)
                out_amp = np.squeeze(out_amp)
                out_amp = (127.5*(out_amp+1)).astype('uint8')
                cv2.imwrite(im_path, cv2.cvtColor(out_amp,code=cv2.COLOR_RGB2BGR))
    ...
    def _build_training_graph(self, train_config):
        ...
        filename_queue = tf.compat.v1.train.string_input_producer( [os.path.join(train_config["dataset_dir"], 'train.tfrecords')], num_epochs=train_config["num_epochs"])
        frameA, frameB, frameC, frameAmp, amplification_factor =  read_and_decode_3frames(filename_queue, (train_config["image_height"], train_config["image_width"], self.n_channels))
        min_after_dequeue = 1000
        num_threads = 16
        capacity = min_after_dequeue + (num_threads + 2) * train_config["batch_size"]
        frameA, frameB, frameC, frameAmp, amplification_factor = tf.compat.v1.train.shuffle_batch([frameA,frameB, frameC,frameAmp, amplification_factor],
                batch_size=train_config["batch_size"],capacity=capacity,num_threads=num_threads, min_after_dequeue=min_after_dequeue)
        ...
        self.loss_function = partial(self._loss_function,train_config=train_config)
        self.output = self.image_transformer(frameA,frameB, amplification_factor, [train_config["image_height"], train_config["image_width"]], self.arch_config, True, False)
        ...
        with tf.compat.v1.variable_scope('ynet_3frames/encoder', reuse=True):
        ...
        self.image_sum = tf.summary.image('train_B_OUT', tf.concat([frameB, self.output],axis=2), max_outputs=2)
        if self.n_channels == 3:
            self.image_comp_sum = tf.summary.image('train_GT_OUT',frameAmp - self.output, max_outputs=2)
            self.image_orig_comp_sum = tf.summary.image('train_ORIG_OUT', frameA - self.output, max_outputs=2)
        else:
            self.image_comp_sum = tf.summary.image('train_GT_OUT', tf.concat([frameAmp, self.output, frameAmp], axis=3),  max_outputs=2)
            self.image_orig_comp_sum = tf.summary.image('train_ORIG_OUT', tf.concat([frameA, self.output, frameA], axis=3), max_outputs=2)
  ...
  # Loss function
    ...
    def train(self, train_config):
        ...
        self.lr = tf.compat.v1.train.exponential_decay(train_config["learning_rate"], self.global_step, train_config["decay_steps"], train_config["lr_decay"], staircase=True)
        self.optim_op = tf.compat.v1.train.AdamOptimizer(self.lr, beta1=train_config["beta1"]) .minimize(self.loss, var_list=tf.compat.v1.trainable_variables(),
                    global_step=self.global_step)
        ginit_op = tf.compat.v1.global_variables_initializer()
        linit_op = tf.compat.v1.local_variables_initializer()
        self.sess.run([ginit_op, linit_op])
        self.writer = tf.compat.v1.summary.FileWriter(train_config["logs_dir"], self.sess.graph)
        coord = tf.train.Coordinator()
        threads = tf.compat.v1.train.start_queue_runners(sess=self.sess, coord=coord)
        start_time = time.time()
        for v in tf.compat.v1.trainable_variables():
            print(v)
        if train_config["continue_train"] and self.load(train_config["checkpoint_dir"]):
            print('[*] Load Success')
        elif train_config["restore_dir"] and self.load(train_config["restore_dir"], tf.compat.v1.train.Saver(var_list=tf.compat.v1.trainable_variables())):
        ...
        try:
            while not coord.should_stop():
                ...
                if global_step % 100 == 0:
                    # Write image summary.
                    img_sum_str, img_comp_str, img_orig_str = self.sess.run([self.image_sum, self.image_comp_sum, self.image_orig_comp_sum])
